...
def draw():
    ...
    canvas.create_text(1417.5,150,text="©Denis Dinga™", angle=-90, font = 'Georgia 20 bold')
...
def Z_release():
    global Z_pressed
    if keyboard.is_pressed("z") == False:
        print(pygame.KEYDOWN)
...
def X_release():
    global X_pressed
    if keyboard.is_pressed("x") == False:
...
def The_one(sur):
    if keyboard.is_pressed("z") == True:
        ...
        Z_thread(sur)
    if keyboard.is_pressed("z") == False:
        Z_release_thread(sur)

    if keyboard.is_pressed("x") == True:

        X_thread(sur)
    if keyboard.is_pressed("x") == False:
        X_release_thread(sur)

    if keyboard.is_pressed("c") == True:

        C_thread(sur)
    if keyboard.is_pressed("c") == False:
        C_release_thread(sur)

def The_one_thread(sur):
    t48 = threading.Thread(target=The_one(sur))
    t48.start()


#win.bind("<KeyPress->",The_one_thread)
#win.bind("<KeyRelease->",The_one_thread)
